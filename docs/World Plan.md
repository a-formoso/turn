...
- **A 360° equirectangular panorama per location** ("the world plate"),
  generated by the image model from the location's spec + depth grid
  (equirect panos are a known-good prompt pattern for Nano Banana 2; the
  proxy already relays aspect/size).
