...
const APP_FEATURES = [
  ...
  { name:"Script view", what:"each scene's screenplay, generated from its beats; the left gutter shows the beat each block expands. The page follows shooting-script conventions: the scene number flanks the slugline in both margins, a speaker returning after intervening action gets (CONT'D) automatically, and transition lines (CUT TO:, FADE OUT.) are detected and set right. You can EDIT the screenplay directly: an 'Edit' button (pencil) in the script toolbar turns every block — slugline, action, character cue, parenthetical, dialogue, transition — into an editable line you type straight into; changes save when you click away from a block ('Done editing' leaves edit mode). Each edit is committed as a new version, so the Undo / Redo buttons step back and forth through your manual edits (and MUSE drafts/polishes) on one shared history." },
...
async function aiScriptBreakdown(scene, beats, draft, characters){
  ...
  const prompt =
    ...
    '{"characters":[{"name":"<exact cast name>","gender_used":"he|she|they|unclear","features":["physical/anatomical detail the scene reveals, e.g. forearm maintenance panel"],"states":["appearance change in this scene, e.g. forearm panel open, wires exposed"]}],'+
...
async function aiDraftShots(scene, beats, drafts, locations, props, characters, project){
  ...
  const prompt = ctx + "\nYou are the director + DP. Break this scene into a SHOT LIST with exactly ONE shot per beat above"+
    ...
    "action = one vivid present-tense sentence of what we SEE in this frame. "+SELF_CONTAINED_BEATS+
