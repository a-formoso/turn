...
if(_shotsFeature) _shotsFeature.what += " Micro-beat checklist text is preserved as readable action prose rather than clipped to a tiny label; long micro-beats are word-boundary shortened only after enough room to keep the filmed action intelligible.";
...
async function aiDraftShots(scene, beats, drafts, locations, props, characters, project, opts){
  ...
  try{
    ...
    const microText = (t)=> (typeof clipWords==="function")
      ? clipWords(scrubBrand(String(t||"")).replace(/^\s*\d+[\.\)]\s*/,"").replace(/\s+/g," ").trim(), 180)
      : scrubBrand(String(t||"")).replace(/^\s*\d+[\.\)]\s*/,"").replace(/\s+/g," ").trim().slice(0,180);
