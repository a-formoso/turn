...
async function aiAssignSceneStyles(scenes, seedPresets, drafts, project){
  ...
  const prompt = ctx +
    ...
    'Return ONLY compact JSON: {"presets":[{"id":"...","name":"...","grade":"...","palette":["#..","#..","#.."],"dominantLabel":"...","secondaryLabel":"...","accentLabel":"...","lighting":"...","lens":"...","texture":"..."}],"sceneStyles":{"<sceneNo>":"<presetId>", ...}}';
  try{
    const res = await window.claude.complete({ messages:[{ role:"user", content:prompt }] });
    const j = extractJSON(res);
    if(!j) return null;
