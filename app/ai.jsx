...
const APP_FEATURES = [
  ...
  { name:"The Stage (Production)", what:"the third ROOM (after the Writers' Room and the Art Room), labelled 'The Stage' — Production phase, where the film becomes generated VIDEO. The Stage is built as a SEEDANCE 2.0 DIRECTOR CONSOLE. The unit of work is the CLIP — one clip sequence from the Shot List (a contiguous run of shots ≤15s, the Stage's render unit, labelled per scene like 01A / 01B / 01C) — and each clip is exactly ONE Seedance 2.0 video generation. Layout: a left SCENES rail expanded into its clips (with duration and a dot when a clip has been rendered), plus a Board / Timeline workspace. Board groups clip cards by scene in cut order, showing each clip's frame, voice and video readiness plus per-scene progress. The Stage's left rail, board cards and shot filmstrip prefer the actual screenplay text assigned to each beat, then the shot action/dialogue drafted from that beat, falling back to beat-map labels only when neither script nor shot text exists, so the production labels stay matched to the written script. The selected clip has a Visual source switch: Shot frames uses the individual Shot List start frame; Sheet halves uses the saved halves from the matching 2x2 Storyboards sheet (`Save halves for the Stage`) or the full scene sheet as Seedance image inputs, so a GPT Image 2/uploaded storyboard sheet can drive video continuity without relying on independently generated shot frames. The Stage is responsive: on narrower screens the rail becomes a compact top strip, the render console stacks above the Seedance controls, and asset cards resize into a touch-friendly grid. The Stage is format-aware: its player, board cards, shot filmstrip and Seedance generation payload use the project's native aspect ratio (for example 16:9 film/short/series/commercial/documentary or 9:16 micro-drama), and the header shows that ratio. Timeline opens the selected clip in the render console. For the selected clip the console shows: a video PLAYER (the rendered clip, or the selected visual source — its first shot frame or saved storyboard half/sheet — as a poster until rendered) with a filmstrip of the scene's clips; a 'Seedance 2.0 Director' PRIMARY PROMPT (a composed multi-beat prose prompt built screenplay-first from the clip's mapped beat text, with storyboard panel action included when it differs, plus camera arc + the scene's style grade, editable) with a row of PROMPT RECIPES — buttons that rewrite the editable prompt in different ways tuned to how Seedance 2.0 responds: 'Narrative' (one flowing paragraph, the default), 'Panel-by-panel' (one line per beat/panel like '[Panel 1.2] …', folding in each dialogue line's voice DURATION since the voice may begin mid-panel, plus anti-hallucination rules — no fixed timeline so Seedance paces the cut, and max ~2–3 characters per frame), 'Structured' (labelled Subject / Action / Setting / Camera / Lighting / Style fields), and 'Dialogue / lip-sync' (phrased around the spoken line in @Audio1). Picking a recipe rewrites the prompt; you can still hand-edit after. There are also derived clickable prompt-ingredient chips (subject / action / setting / camera / style / mood) that append the exact ingredient back into the prompt; and an INPUT ASSETS row of the references Seedance combines (Seedance takes up to ~12 assets) — all DERIVED automatically: the prompt (@text1), the selected visual source (only the storyboard half/halves that cover that clip when Sheet halves is selected), the in-frame characters' sheets (@image1…), the location plate, in-frame/carry-forward prop sheets, the previous clip (for continuity, as a @video reference) and the locked-voice line audio (@audio1). The row labels every asset as Required, Included, Excluded, or Missing: required assets such as the prompt, selected visual source and locked voice cannot be toggled; optional ready references can be toggled in/out and the generate payload follows that state exactly; missing source assets stay visibly unavailable until generated. Stage shows a Seedance input budget meter in the asset row and right panel (selected / 12, slots left or over cap), and Generate is disabled with an explicit warning until the selected inputs are back within the 12-asset budget and the selected visual source exists. A right-hand SEEDANCE PANEL has the model capability card, a MODE selector (Text-to-Video / Image-to-Video / Video-to-Video / Multimodal — auto-picked from which asset kinds are toggled on, overridable), an ASSETS SUMMARY count (Text/Images/Videos/Audio), and CONTROLS: Camera movement (derived from the clip's shot moves), Lighting (from the scene's style grade), Performance (the in-frame lead/speaker), Audio status, Quality mode (Standard / Turbo — Turbo uses the faster Seedance model), and read-only Clip duration. Dialogue clips are audio-first: until their locked-voice line audio exists, they show 'needs voice', have no duration yet, and cannot render video. Once voice exists, duration is locked to the measured voice clock. Silent clips can render with native audio and the clip's estimated length. 'Voice this clip' renders only the selected clip's missing dialogue lines, while 'Voice all lines' renders every dialogue line in the film; both use locked character voices and update measured durations before video. 'Generate clip' renders the clip; 'Extend / edit' re-generates using the existing clip as a video reference (Seedance's iteration/extension path). Video runs SERVER-SIDE through the fal.ai proxy (FAL_KEY as a server secret, the image-proxy 'video' route), so it needs the proxy deployed + you signed in — until then Generate shows a friendly 'video proxy isn't deployed' message (same pattern as voice). The @audio1 line-audio asset comes from the locked voice (locked per character on the character card, Characters tab, and rendered through the ElevenLabs proxy route). The room is locked until a story exists and shows an empty state pointing to the Shots tab if no shots are designed yet." },
