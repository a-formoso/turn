...
const APP_FEATURES = [
  ...
  { name:"Worlds (Locations \u2014 scout & frame inside the space)", what:"each location card has a WORLD fold \u2014 the location-consistency layer: instead of only fixed reference plates, a location can become ONE CONTINUOUS SPACE that shots are framed inside. Two engines (server-side keys, set by the studio): 'Create 360\u00b0 world' renders a 360\u00b0 panorama world plate of the location from its written spec (~30-60s); 'Create 3D world' builds a fully explorable 3D world (~5 minutes) \u2014 it also yields the 360\u00b0 plate, plus a downloadable 3D gaussian-splat file linked on the card. Once a world exists, 'Scout this location' opens the SCOUT: a viewer where you stand inside the world plate \u2014 drag to look around, mouse wheel to change the lens \u2014 and the canvas IS the frame, sized to your format's aspect (16:9, 9:16\u2026). 'Take view' captures the current framing as a named, reusable VIEW saved on the location (View 1, View 2\u2026, click a view chip to open it) \u2014 these views are the geometry truth for shots set in that location, so returning to the same place always shows the same space. If the buttons report a missing server key, the studio needs to set BLOCKADE_API_KEY (skybox.blockadelabs.com/api) or WORLDLABS_API_KEY (platform.worldlabs.ai/api-keys) on the proxy." },
