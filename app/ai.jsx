...
async function aiDraftShots(scene, beats, drafts, locations, props, characters, project, opts){
  ...
  const prompt = ctx + "\nYou are the director + DP. "+
    (_onlyBeat
      ...
      : ("Break this scene into a full SHOT LIST as a BEAT-BY-BEAT MAPPING: every beat above becomes 1-4 shots of REAL COVERAGE \u2014 one shot per DISTINCT VISUAL EVENT in that beat, never more than the beat's text supports (a beat carrying one simple visual event stays ONE shot; padding with redundant setups is worse than too few). The beat's DRIVE and REACTION usually want separate setups when both carry action; a small continuity object handled at close range earns an INSERT. Keep every shot in cut order"+
