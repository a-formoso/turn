...
const APP_FEATURES = [
  ...
  { name:"Storyboard (Art Room)", what:"turns the film into professional storyboard SHEETS: each SCENE becomes a sheet with a header bar (PROJECT · SCENE · TITLE · PAGE) over ONE composite storyboard image (scenes with more than 12 shots paginate, e.g. 'PAGE 1 of 3'). The sheet image is drawn by GPT Image 2 in a SINGLE pass at 16:9, built on the 'Cinematic Storyboard Grid' template: an R×C grid of the scene's panels rendered as ONE CONTINUOUS take (read left-to-right / top-to-bottom — the camera moves through one unbroken moment, not unrelated images), with a thin off-white annotation strip baked UNDER each panel carrying three screenplay slug lines — CAM (framing & camera move), MOVE (the subject's action), and a third line that is MOOD (atmosphere) by default or VOICE (the spoken line) on dialogue panels. Characters are kept identical across panels via a tight one-line 'character lock' per cast member PLUS the attached reference sheets (location plate + up to four character sheets — the set is kept lean so the single heavy image doesn't time out), and the scene's Style-Bible grade + film stock are applied so the look is consistent. An un-generated sheet is a clickable placeholder ('Generate single sheet'); once generated it has a '...' menu (View full / Details / Edit sheet — a text instruction to change the whole sheet / Regenerate / Clear). Each sheet also has a 'Beat & shot briefs' expander with the full per-panel detail from the Shot List + Writers' Room beat/subtext map (driver/reactor, desire, antagonism, and per panel: who's in frame, action, composition, dialogue, the driver's & reactor's subtext, and the shot's final frame prompt); an 'Edit' link jumps to that shot in the Shot List. 'Generate all sheets' batch-renders every scene's sheet one at a time (progress count + cancel); a readout shows 'X of Y sheets generated'. 'Direct storyboard' hands the whole board to the STORYBOARD DIRECTOR agent — an autonomous agent that boards the film as a CONTINUOUS visual narrative: for each scene it asks the writing model to think the panels through (using everything in the Beat & shot briefs) into an optimized single-sheet prompt, then renders the sheet with GPT Image 2 — chaining the PREVIOUS scene's rendered sheet in as a visual anchor and carrying a running 'continuity memo' so the cast looks, world and colour grade stay consistent scene to scene. It runs scene after scene with a live step-by-step trace and a Stop button (no per-sheet approval; rendered sheets are revised per-card via Regenerate/Clear). 'Export storyboard' opens a printable black-background document with each scene's header + sheet image (Save as PDF). This is the last Art Room tab and the hand-off point to the Stage (the upcoming voice/video step)." },
