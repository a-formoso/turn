...
function _wornCarriedSplit(e, character, clean){
  ...
  let accRaw = String((e&&e.accessories)||"").trim();
  let propRaw = String((e&&e.props)||"").trim();
  if(!hasAcc && accRaw && !/^none$/i.test(accRaw)){
    ...
    if(move.length){
      accRaw = keep.join(", ") || "none";
      const pl = (propRaw && !/^none$/i.test(propRaw)) ? propRaw.toLowerCase() : "";
      // duplicate test compares OBJECT NOUNS only ("smartphone" vs an existing
      // "phone" = same thing; "tote" vs "file" = different) — never loose words,
      // which once false-matched "with" and stranded a tote
      ...
      if(fresh.length) propRaw = (pl ? (propRaw+", ") : "") + fresh.join(", ");
    }
  }
  return {
    accessories: hasAcc ? character.accessories : clean(accRaw,160),
    props: hasProps ? character.props : clean(propRaw,220),
  };
}
