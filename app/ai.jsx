...
const APP_FEATURES = [
  ...
  { name:"Home \u2014 the studio dashboard", what:"clicking the Cinema Machine logo in the top-left opens HOME, a studio front-door dashboard — but ONLY once you own 2 or more films; with 0 or 1 film the logo takes you straight into the Writers' Room instead (there's nothing to overview yet). The dashboard has: a HERO that pitches the studio and shows a production PIPELINE STEPPER (01 Writers Room / 02 The Art Room / 03 The Stage / 04 Post — coming soon) — clicking a live step jumps into that room through the app's normal room gate, and the current step is highlighted; the hero's media panel shows the current (or most recent) film's poster with a play button into The Stage. Below that is a RECENT PROJECTS strip: up to five of your films, newest-UPDATED first, each a card with its poster (or the title's initials until one exists), a format pill (Short Film, Feature Film, TV Series, Commercial, Micro-Drama, Documentary), the title and an 'Updated Xd/h/m ago' stamp; clicking a card opens that film, and a '+ New Project' tile creates a blank film. A top bar carries the brand (click to go back to your current film), the current-project chip, a '+ New Project' button and your account menu; a feature strip sits at the bottom. MOVIE POSTERS are generated AUTOMATICALLY: any recent film without key art gets one minted on the spot (a 'Painting poster…' spinner shows on the card), painted with GPT Image 2 through the server image proxy (no key in the browser), then saved onto the film (doc.cover) so it's only ever painted ONCE and syncs across devices. Every poster is grounded in the film's OWN canon, never invented: up to three lead character portrait sheets ride along as references so the poster shows the ACTUAL cast (faces and wardrobe faithful — a film with no cast gets an evocative character-free poster instead); the film's primary LOCATION plate rides along so the setting is this film's real world, not an invented backdrop; a signature PROP may feature if it strengthens the composition; and the poster is rendered in the film's own MEDIUM — its dominant picked render style (Pixar, anime, ukiyo-e…) — so an animated film gets an animated one-sheet while photoreal films get a live-action one-sheet. For a show episode all of this reads from the show's shared bible. Every poster is text-free (no title/lettering). HOVERING a project card reveals POSTER ACTIONS: Download (saves the poster as a JPG), Regenerate (paints a fresh poster — the outgoing one is kept as one-deep history), and Restore (swaps back to the previous poster; shown only when one exists). When you own more than five films, ‹ › ARROWS next to the 'Recent Projects' title page through all of them (a full window of five, sliding to the end). The '+ New Project' tile appears in the strip while you own fewer than five films; beyond that the orange top-bar button covers creating films. The dashboard ends with the same footer as the public homepage (Privacy / Terms / Contact). It is CREDIT-SAFE and fail-closed: if the cast can't be read (a network/auth blip) it skips that film WITHOUT spending and retries on the next visit rather than baking in wrong art, an in-flight guard prevents painting the same poster twice, and if a render fails (e.g. you're out of image credits) the batch stops with a message instead of burning attempts. Shows (series containers) aren't listed — only openable films. The project SWITCHER dropdown next to the title still does quick in-place switching, rename, new-episode, make-a-show and per-film delete." },
