...
/* ============================================================
   ...
   ============================================================ */
const APP_FEATURES = [
  { name:"Spine view", what:"a graph of every scene's value charge end-to-end; each point is clickable, and you can drag to pan across the acts." },
  ...
  { name:"Script view", what:"each scene's screenplay, generated from its beats; the left gutter shows the beat each block expands." },
  ...
  { name:"Agents", what:"the 'Agents' button in the top bar opens a panel of AI agents that REFINE an existing story: Story Doctor (finds the weakest structural link and proposes a fix, re-auditing until the spine holds), Continuity Repair (plants missing setups and pays off dangling threads, re-checking each time), and Table-Read (whole-script pacing/tone/voice critique). Each shows its reasoning and asks approval before changing anything. (MUSE is NOT in this panel — MUSE is the separate floating help assistant in the bottom-right corner; the Agents CHANGE your story, MUSE just answers questions.) Creating a story from scratch is NOT here — that's the 'New Story' button, which develops your idea into a logline and architects the full spine (it uses the same builder under the hood, so there's exactly ONE way to start a story). The 'Agents' panel is distinct from the 'Writers' Room', which is the story-development ROOM (spine/script) in the room switcher." },
  { name:"Undo agent changes", what:"after an agent applies changes, a floating Undo control (and a row in the Agents panel) lets you revert that run's changes to the whole story in one click; the last several runs are kept so you can undo them in turn." },
  { name:"MUSE (help assistant)", what:"the friendly AI guide to TURN — a floating chat bubble in the BOTTOM-RIGHT corner, available in every room. Click it to open a chat box and TYPE a question about your story, any department, the Infinite Studio method, or how to get something done; MUSE answers concisely in text and remembers the conversation. MUSE only answers questions — it never changes your story (that's what the Agents do), and it's deliberately separate from the Agents panel. MUSE will not discuss what powers it or how TURN is built." },
...
/* ---- STYLE BIBLE: read the whole film and assign each scene a style preset.
   Returns { sceneStyles: {sceneId: presetId}, presets?: [...] }. The model picks
   from the supplied preset list (it may also propose up to 2 bespoke presets). ---- */
async function aiAssignSceneStyles(scenes, presets, drafts, project){
  if(!aiAvailable() || !scenes || !scenes.length) return null;
  const P = project || (window.TURN_DATA||{}).PROJECT || {};
  let ctx = "FILM: "+(P.title||"Untitled")+" \u2014 "+(P.genre||"")+". Logline: "+(P.premise||"")+"\n\n";
  ctx += "AVAILABLE STYLE PRESETS (id \u2014 name: grade):\n";
  (presets||[]).forEach(p=>{ ctx += "- "+p.id+" \u2014 "+p.name+": "+(p.grade||"")+"\n"; });
  ctx += "\nSCENES (number | title | what happens):\n";
  scenes.forEach(s=>{
    const script = (typeof sceneScriptText==="function" ? sceneScriptText(s.id, drafts) : "").replace(/\s+/g," ").slice(0,300);
    ctx += "#"+s.no+" "+(s.title||"")+" \u2014 "+(s.summary||"").replace(/\s+/g," ").slice(0,180)+(script?(" | "+script):"")+"\n";
  });
  const prompt = ctx + "\nAssign EACH scene the ONE preset id whose look best serves its dramatic function, "+
    "world/setting and emotional tone. Keep visually/tonally related scenes on the same preset for continuity; "+
    "let the look shift only when the story shifts. You MAY define up to 2 bespoke presets if the film clearly needs "+
    "a look the list lacks \u2014 give each an id, name, grade, a 3-colour palette (dominant, secondary, accent as hex, the 60/30/10 split), "+
    "lighting, lens and texture.\n"+
    'Return ONLY compact JSON: {"sceneStyles":{"<sceneNo>":"<presetId>", ...}, "newPresets":[{"id":"...","name":"...","grade":"...","palette":["#..","#..","#.."],"dominantLabel":"...","secondaryLabel":"...","accentLabel":"...","lighting":"...","lens":"...","texture":"..."}]}';
  try{
    const res = await window.claude.complete({ messages:[{ role:"user", content:prompt }] });
    const j = extractJSON(res);
    if(!j) return null;
    ...
    const sceneStyles = {};
    const raw = j.sceneStyles || j.scenes || j;
    if(raw && typeof raw==="object"){
      Object.keys(raw).forEach(k=>{ const id = noToId[String(k).replace(/[^0-9]/g,"")] || (scenes.find(s=>s.id===k)?k:null);
        if(id && raw[k]) sceneStyles[id] = String(raw[k]); });
    }
    const newPresets = Array.isArray(j.newPresets) ? j.newPresets.filter(p=>p && p.id && p.name).map(p=>({
      id: locSlug ? locSlug(p.id) : String(p.id), name:String(p.name).slice(0,40), grade:String(p.grade||"").slice(0,120),
      palette: Array.isArray(p.palette)?p.palette.slice(0,3):[], dominantLabel:p.dominantLabel||"", secondaryLabel:p.secondaryLabel||"", accentLabel:p.accentLabel||"",
      lighting:String(p.lighting||"").slice(0,120), lens:String(p.lens||"").slice(0,80), texture:String(p.texture||"").slice(0,100),
    })) : [];
    return { sceneStyles, newPresets };
  }catch(e){ return null; }
}
