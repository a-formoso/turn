...
if(_shotsFeature) _shotsFeature.what += " Every shot card shows the SCRIPT BEAT it covers — a quoted 'Script beat' line (the beat's drive — reaction text from the scene's beat map) right under the card's head row, so the source text rides with the shot; it's read-only there (beats are edited in the Writers' Room inspector). Every shot keeps its beat number, but shots are NOT one-to-one with beats: when a scene is designed, each beat is decomposed into its micro-beats and covered by AS MANY SHOTS as its distinct visual events need (uncapped — a simple beat may stay one shot, a busy beat gets several; an optional establishing wide can open beat 1), so a re-design yields real beat-by-beat coverage, not a single shot per beat.";
if(_shotsFeature) _shotsFeature.what += " Coverage is also backstopped after every scene Re-draft: if the writing model returns shots for only some Writers' Room beats, TURN still shows one beat card per source beat and inserts a simple coverage shot for any missing beat; if a returned micro-beat is marked uncovered, TURN adds a matching shot for that micro-beat before saving the redesigned scene. The amber uncovered tag remains useful for legacy shots or manual edits, but a fresh Re-draft is not allowed to silently drop beats or knowingly save uncovered micro-beats.";
...
if(_locationsFeature) _locationsFeature.what += " The Locations toolbar has a World scale control with icon options: \u2728 Auto (by occupant, the default) then \ud83e\uddcd Human (Class A), \ud83d\udc01 Critter (Class B), \ud83d\uddff Giant (Class C), \ud83d\udd2c Microscopic (Class D); Microscopic forces location plates into the Class D molecular/cellular world scale. Each location CARD also shows a small resolved-scale badge (e.g. \ud83d\udc01 B) whenever its plate will render at a non-human scale \u2014 the tooltip says whether that came from the project-wide control or was derived automatically from the characters who drive that place's scenes; plain human-scale locations show no badge.";
...
if(_shotOrderFeature) _shotOrderFeature.what += " Shot location references resolve by source quality: when a generated slugline-unit plate exists for the selected shot's scene/side/time (for example a Minicab INT unit or Minicab EXT coverage unit), the Shots tab uses that full-resolution unit plate as the location anchor instead of cropping a quadrant from the master 2x2 location plate. Screenplay-derived coverage sheets are next in priority; cropped master-location panels are only the fallback when no matching full-frame unit or coverage sheet is available.";
if(_shotOrderFeature) _shotOrderFeature.what += " Shot reference assets are explicitly tagged for provenance: full character/prop/location sheets are master references, slugline units and screenplay coverage sheets are derived references, cropped master panels are crop fallbacks, generated shot frames are shot anchors, and rendered Stage clips are final takes. The resolver ranks those roles so lower-resolution crop fallbacks never beat a valid full-frame unit/coverage source.";
if(_shotOrderFeature) _shotOrderFeature.what += " The location-reference resolver now tries all direct full-frame slugline-unit and screenplay coverage plates before accepting any master-plate crop fallback; if a generated full unit/coverage plate exists, both the reference preview and the generation payload suppress the lower-resolution crop.";
  ...
  const prompt = ctx + "\nYou are the director + DP. "+
    (_onlyBeat
      ? ("Beat "+_onlyBeat+" needs REAL COVERAGE: design 2-4 shots for THAT BEAT ONLY \u2014 one per DISTINCT VISUAL EVENT, never more than the beat's text supports (padding coverage with redundant setups is worse than too few), in cut order. "+
         "The beat's DRIVE and REACTION usually want separate setups when both carry action; a small continuity object handled at close range earns an INSERT; "+
         "give the most expressive setup to the moment that shifts the beat. Every other beat already has its own coverage \u2014 return ONLY beat "+_onlyBeat+"'s shots, each with beat="+_onlyBeat+". ")
      : ("Break this scene into a full SHOT LIST as a BEAT-BY-BEAT MAPPING. FIRST decompose EACH beat into its MICRO-BEATS: every discrete filmable action in order (2-7 per beat, numbered from 1 WITHIN that beat) — then design the shots so EVERY micro-beat is covered by at least one shot. Also name, per beat, what the camera must PROTECT: one sentence stating the beat's emotional core (the moment that carries its meaning). Every beat above becomes AS MANY SHOTS OF REAL COVERAGE as its DISTINCT VISUAL EVENTS genuinely need \u2014 no numeric cap, but every shot must earn its place as its own visual event (a beat carrying one simple event stays ONE shot; padding with redundant setups is worse than too few). The beat's DRIVE and REACTION usually want separate setups when both carry action; a small continuity object handled at close range earns an INSERT; a held look, a glance back, or a reveal can each be their own setup when the drama rests on them. The scene MAY also open on one ESTABLISHING WIDE of the space before anyone acts (assign it to beat 1) when geography helps the cut. Keep every shot in cut order"+
    ...
    "covers = array of the micro-beat numbers (within this shot's beat) this shot covers. "+
