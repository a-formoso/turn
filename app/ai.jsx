...
const WRITING_MODELS = [
  ...
  { id:"claude-opus-4-8",    label:"Claude 4.8",      provider:"anthropic", note:"richest screen prose — best for scripts & scenes" },
...
const WRITING_TASK_DEFAULTS = { story:"claude-opus-4-8", specs:"gemini-3.5-flash" };
...
const APP_FEATURES = [
  ...
  { name:"Export (screenplay / story)", what:"the Export button in the top bar — a Writers' Room action (it exports the screenplay and story, so it only appears there, not in the Art Room; the Art Room has its own per-tab exports like the shot list and storyboard). Four formats: Screenplay (PDF) — opens an in-app preview first, then Print / Save as PDF or Download .html; Screenplay (.fountain) — opens in Final Draft, Highland and other screenwriting apps; Story outline (.txt) — premise, controlling idea & spine; Spine data (.csv) — scenes, charges and turns. There is no share-to-WhatsApp/email — exporting produces files. (The top bar's overflow ⋮ menu is ADMIN-ONLY — except in the ART ROOM, where it shows for everyone and carries 'Export shot list'; for everyone else the top bar shows exactly: theme toggle, Brief, New Story, Export — when a drafted story exists, in the Writers' Room — and the account chip. JSON, Styles, API Keys and ⋮ are admin tools.)" },
...
async function aiApplyQaAdvice(noun, fields, advice){
  ...
  try{
    ...
    keys.forEach(k=>{ if(typeof j[k]==="string" && j[k].trim() && j[k].trim()!==String(fields[k]||"").trim())
      patch[k] = scrubBrand(j[k].trim()).slice(0, 600); });
...
async function aiDraftShots(scene, beats, drafts, locations, props, characters, project, opts){
  ...
  const prompt = ctx + "\nYou are the director + DP. "+
    ...
    _lookbookBlock(P)+
    'Return ONLY compact JSON: {"beats":[{"beat":1,"micro":["...","..."],"protect":"..."}],"shots":[{"beat":1,"size":"WS","angle":"eye","move":"static","lens":"35","subjects":["..."],"props":[],"action":"...","composition":"...","dialogue":"...","covers":[1,2],"purpose":"...","priority":false}]}';
  try{
    const res = await window.claude.complete({ messages:[{ role:"user", content:prompt }] });
    const j = extractJSON(res);
