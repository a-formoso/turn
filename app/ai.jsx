...
APP_FEATURES.push({ name:"Writing engine dock", what:"a floating pill on the right edge of the Writers' Room and Art Room (stacked above the Art Room's image-engine dock) showing the current WRITING engine — AUTO by default. Click it to open the engine panel — grouped exactly like the image-engine dock: a MODE row (Auto \u00b7 per task) then compact segmented buttons under PROVIDER labels (Anthropic / Google / OpenAI / Moonshot), \u2605 marking the story/specs recommendations, with the selected engine's note shown beneath. AUTO follows the per-task recommendation (story prose \u2192 the richest-prose engine; Art-Room specs \u2192 the fast engine), and picking a specific model (Claude 4.8, Claude Fable 5, Gemini 3.5 Flash, GPT-5.5, or KIMI K3 \u2014 Moonshot's brand-new 2.8-trillion-parameter deep reasoner: huge context and a distinct voice, but SLOW on long drafts (a full scene redraft can hit the server's time window, especially at peak) \u2014 best for analysis, alt lines and shorter passes; heavy drafting belongs on Auto) pins every drafting call to it for THIS SESSION; every new session starts back on Auto. The dock, the New Story header picker and the agent-panel pickers are the same one session pick. Kimi K3 runs server-side like every engine \u2014 it needs the MOONSHOT_API_KEY secret set on the proxy (and one proxy redeploy) before it answers; until then it reports the missing key." });
